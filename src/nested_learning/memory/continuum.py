...
============================================================================
IMPORTANT: MULTI-FREQUENCY UPDATES ARE NOT INTEGRATED
============================================================================

The methods `get_update_levels()`, `accumulate_gradients()`, and
`apply_accumulated_gradients()` are IMPLEMENTED but NEVER CALLED by any
training loop in this repo.

Currently, CMS is used as a plain sequential MLP stack, completely ignoring
the multi-frequency update concept that is central to the paper.

To actually use multi-frequency updates, your training loop must:
1. Call cms.get_update_levels(step) each step
2. Call cms.accumulate_gradients(level) for each level
3. Call cms.apply_accumulated_gradients(level) when level should update

See IMPLEMENTATION_STATUS.md for full discussion of what's missing.
============================================================================
"""
